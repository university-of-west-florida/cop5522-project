...
void samplesort(int inputNums[], int start, int end, int numOfBuckets, int samplesPerBucket) {
    std::cout << "start: " << std::to_string(start)
        ...
        << ", numOfBuckets: " << std::to_string(numOfBuckets)
        << ", samplesPerBucket: " << std::to_string(samplesPerBucket) << std::endl;
    int i, j, k;
    ...
    if (numOfBuckets * samplesPerBucket > numNums) {
        // Unsure if this is supposed to have equality flipped
        ...
        // If there are fewer inputNums than specified numOfBuckets, then just bubblesort the rest bc it is likely a small set left
            ...
            std::cout << "bubblesort function end" << std::endl;
            return;
        }
    }
    ...
    for (i = 0; i < numOfBuckets; ++i) {
        ...
        std::cout << "Calling new samplesort" << std::endl;
...
void* samplesort(void* vArgs) {
    ...
    for (i = 0; i < numOfBuckets; ++i)
    {
        for (j = start + (i * (numNums / numOfBuckets)); j < start + ((i + 1) * (numNums / numOfBuckets)); ++j)
        {
            ...
            // Start up thread
            pthread_create(newThread, NULL, testFunc2, static_cast<void*>(& tArgs));
