...
//-------------------------------------------//
	...
	if (prepState == 1) {
		...
		if (proc_num > 0)
			partialArray = (int*) malloc( (numNums/comm_size) * sizeof(int));
		else {
			partialArray = (int*) malloc( ((numNums/comm_size) + (numNums % comm_size)) * sizeof(int));
...
// PRINTING PARTIAL ARRAYS //
/*
		if (proc_num == 0) {
		    	printf("Full array Values:\n\t");
	    		for(int i = 0; i < numNums; ++i)
	        		printf ("%i ",inputSS[i]);
	    		printf("\n\n");

		    	printf("Array Values, proc%i:\n\t",proc_num);
	    		for(int i = 0; i < (numNums/comm_size) + (numNums % comm_size); ++i)
		    	...
		    	printf("Array Values, proc%i:\n\t",proc_num);
	    		for(int i = 0; i < (numNums/comm_size); ++i)
	        		printf ("%i ",partialArray[i]);
	    		printf("\n\n");
		}
*/
		...
		if(proc_num > 0)
			make_partial_buckets(partialArray, numNums/comm_size, comm_size, splitters, B, B_sizes);
		else
			make_partial_buckets(partialArray, (numNums/comm_size) + (numNums % comm_size), comm_size, splitters, B, B_sizes);

// PRINTING Splitters & B_Sizes //
/*
...
/*
			if (proc_num == i) {
	    			printf("Rcv sizes, proc%i:\n\t",proc_num);
  				for(int j = 0; j < comm_size; ++j)
	       				printf ("%i ",rcv_sizes[j]);
    				printf("\n\n");
			}
*/
...
// PRINTING Bucket sizes + Displacements //
/*
	    			printf("proc%i, Bucket size: %i, Displacement:\n\t",proc_num, bucket_size);
  				for(int j = 0; j < comm_size; ++j)
	       				printf ("%i ",displc[j]);
    				printf("\n\n");
*/
